...
const CORE_EXERCISES: &[Exercise] = &[
    ...
    Exercise {
        id: "generic-largest",
        lesson_id: "generics-traits",
        title: "Candidate::new 的 path-like 约束",
        kind: ExerciseKind::SingleChoice,
        prompt: "ripgrep 的 Candidate::new 想接收任意 path-like 输入，P 至少需要实现什么能力？",
        code: "pub fn new<P: AsRef<Path> + ?Sized>(path: &P) -> Candidate<'_> {\n    Candidate::new_candidate(path.as_ref(), false)\n}",
        options: &["AsRef<Path>", "Iterator", "Default"],
        answer: Answer::Choice(0),
        explanation: "`AsRef<Path>` 表示 P 可以被借成 Path；ripgrep 因此能接收 Path、PathBuf、str 等多种 path-like 输入。",
        hint: "看函数体里真正调用了什么能力：这里是 `path.as_ref()`。",
    },
    Exercise {
    ...
    Exercise {
        id: "impl-trait-param",
        lesson_id: "generics-traits",
        title: "Display 约束让错误消息可格式化",
        kind: ExerciseKind::FillBlank,
        prompt: "ripgrep 把错误消息转成字符串时，泛型参数 T 需要什么 trait bound？",
        code: "fn error_message<T: ____>(message: T) -> io::Error {\n    io::Error::new(io::ErrorKind::Other, message.to_string())\n}",
        options: &[],
        answer: Answer::Text("Display"),
        explanation: "`Display` 提供用户可读格式化能力；`message.to_string()` 依赖这个能力把泛型值变成错误文本。",
        hint: "`{}` 格式化和 `to_string()` 都对应用户可读的 Display 能力。",
    },
    Exercise {
